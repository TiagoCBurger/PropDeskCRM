...
`VPS_HOST` / `VPS_USER` / `VPS_SSH_KEY`, simplesmente não faz nada. Dual-write
e cópia dos arquivos antigos ainda não existem — ligar R2 vale para objetos
novos. O caminho de atualização na VPS continua sendo `update.sh`.
