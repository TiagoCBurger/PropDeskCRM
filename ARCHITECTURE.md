...
- **Storage (Supabase Storage → Cloudflare R2)**: hoje buckets privados no Supabase (`whatsapp-media`, `ai-policy`, `lgpd-exports`, `skill-assets`) e `brand-logos` público; URLs assinadas. **Roadmap PropDeskCRM:** abstração S3-compatível com destino R2 — ver [`docs/roadmap/migracao-storage-r2.md`](docs/roadmap/migracao-storage-r2.md).
